...
window.GameEngine = (function () {
    ...
    const pool = new Proxy(staticPool, {
      ...
      set(_target, key, value) {
        ...
        if (value?.openWorld) {
          ...
          state.openWorld.nodes[key] = value;
          state.openWorld.nodePool[key] = value;
        }
        return true;
      },
      ownKeys(target) { return [...new Set([...Reflect.ownKeys(target), ...Object.keys(runtime), ...Object.keys(state.openWorld?.nodes || {}), ...Object.keys(state.openWorld?.nodePool || {})])]; },
      getOwnPropertyDescriptor(target, key) {
        const own = Reflect.getOwnPropertyDescriptor(target, key);
        if (own) return { ...own, configurable: true };
        const value = typeof key === "string" ? (runtime[key] || state.openWorld?.nodePool?.[key] || state.openWorld?.nodes?.[key]) : undefined;
  ...
  function locationForState(state, id) { return id ? runtimeLocationPool(state)[id] || null : null; }
  ...
  function rollJourneyOpening(state, intentId) {
    ...
    if (intentId === "tam_su") {
      ...
      return { intentId, type: "sect", targetOrganizationId: faction.id, targetOrganizationKind: "sect", targetName: faction.name, title: "Đứng trước cổng " + faction.name, text: "Ngươi đứng trước cổng " + faction.name + ". Đây là nơi ngươi sẽ thử gia nhập ở giai đoạn 2, không phải một chỗ giữ chỗ tạm thời." };
    }
  ...
  function nurtureFate(state, fateId) {
    ...
    // Nurture only adds relationship points. Stage transitions are earned by
    // the canonical behavior gates in recordFateBehavior/resonateFate.
  ...
  function resonateFate(state, fateId) {
    ...
    drainSan(state, 10, "nghi thức Cộng Minh"); record.stage = 3; record.resonanceUnlocked = true; record.resonanceEffect = fate.resonanceEffect || null; record.resonanceHistory ||= []; record.resonanceHistory.push({ day: gameDayIndex(ensureGameClock(state)), turn: Number(state.meta?.turn || 0), match, effect: record.resonanceEffect });
  ...
  function defyFate(state, fateId) {
    ...
    state.player.fateDefiance = state.player.fateDefiance || {};
    const used = Number(state.player.fateDefiance[fateId] || 0);
    const maxUses = Number(fateAdvancedActionCatalog()?.nghichMenh?.maxUses || 5);
    if (!Number.isInteger(used) || used < 0 || used > maxUses) return { success: false, reason: "Bộ đếm Nghịch Mệnh không hợp lệ.", code: "INVALID_USE_COUNT", used, maxUses };
    if (used >= maxUses) return { success: false, reason: "Mỗi Mệnh chỉ có thể Nghịch Mệnh tối đa " + maxUses + " lần.", code: "MAX_USES", used, maxUses };
    const cost = 15; if (Number(state.player.san || 0) < cost) return { success: false, reason: "Cần Thanh Tỉnh ≥ " + cost, cost };
    drainSan(state, cost, "Nghịch Mệnh");
  ...
  function subLocationWrongness(state, subLocationId = null) {
    const node = D().WORLD_MAP?.locations?.[state?.locationId] || runtimeLocationPool(state)?.[state?.locationId];
  ...
  function computeStats(character) {
    ...
    equippedItemIds(character.equipment).forEach((itemId) => {
      const item = character.generatedItems?.[itemId] || D().ITEMS?.[itemId];
      if (!item) return;
  ...
  function equipmentEligibility(state, itemId, targetSlot = null) {
    const item = itemDefinition(state, itemId);
    ...
    const oldItem = itemDefinition(state, occupiedBy);
  ...
  function equipItem(state, itemId, targetSlot = null) {
    ...
    updateDerived(state);
    pushHistory(state, { type: "sys", text: "§ Đã " + (eligibility.occupiedBy ? "thay " + (itemDefinition(state, eligibility.occupiedBy)?.name || eligibility.occupiedBy) + " bằng " : "trang bị ") + item.name + "." });
    return true;
  }
  ...
  function handleInventoryAction(state, itemId, actionId) {
    const item = itemDefinition(state, itemId); if (!item || !state.inventory[itemId]) return false;
  ...
  function techniqueCatalog() {
    const sourceCatalog = (typeof window !== "undefined" && window.CONG_PHAP_DATA?.techniques) || {};
    const catalog = copy(sourceCatalog);
  ...
  function validateTechniqueCatalog() {
    ...
    const validPathIds = new Set(Object.keys(window.PATH_FATE_RELATIONS?.paths || {}));
  ...
  function updateTechniqueMastery(state, technique, progress, activity) {
    ...
    if (typeof window !== "undefined" && window.GameExpansion?.techniqueEligibility) { const eligibility = window.GameExpansion.techniqueEligibility(state, technique?.id, "training"); if (!eligibility.ok) return 0; }
    ...
    const guildSnapshot = typeof window !== "undefined" && window.GameExpansion?.guildTechniqueSnapshot ? window.GameExpansion.guildTechniqueSnapshot(state, technique) : { masteryGainPct: 0 };
    const gain = Math.max(1, Math.round(base * (1 + Number(state.player.comprehension || 0) / 100) * (1 + Number(guildSnapshot.masteryGainPct || 0) / 100)));
  ...
  function resolveTechniqueCosts(technique, progress, stats, evolution = {}, stance = "steady") {
    const visible = technique.visibleStats || {}, declared = technique.cost || {}, masteryStage = Number(progress.masteryStage || 0), masteryCostMultiplier = masteryStage >= 3 ? 0.85 : 1;
    const cost = (key, legacyKey) => Number(declared[key] ?? visible[legacyKey] ?? 0);
    const manaCost = Math.max(0, Math.ceil(cost("mana", "manaCost") * masteryCostMultiplier * Number(evolution.manaCostMult || 1)));
    const staminaCost = Math.max(0, Math.ceil(cost("stamina", "staminaCost") * masteryCostMultiplier * Number(stats.staminaCostMultiplier || 1) * Number(evolution.staminaCostMult || 1)));
    const sanCost = Math.round(Math.max(0, cost("san", "sanCost") * masteryCostMultiplier * Number(evolution.sanCostMult || 1) * Number(stats.sanCostMultiplier || 1) * (stance === "guarded" ? 0.5 : 1)) * 100) / 100;
    const lifespanCost = Math.round(Math.max(0, cost("lifespan", "lifespanCost") * masteryCostMultiplier * Number(evolution.lifespanCostMult || 1)) * 100) / 100;
    const corruptionCost = Math.round(Math.max(0, cost("corruption", "corruptionCost") * Number(evolution.corruptionCostMult || 1) * (stance === "burst" ? 1.5 : stance === "guarded" ? 0.5 : 1)) * 100) / 100;
    return { manaCost, staminaCost, sanCost, lifespanCost, corruptionCost, dangerous: technique.family === "cam_thuat" || sanCost > 0 || lifespanCost > 0 || corruptionCost > 0 };
  }

  function techniqueFateResonance(state, technique) {
    const turn = Number(state.meta?.turn || 0), pathId = state.player.pathId || state.pathId || null;
    const ids = [...new Set(state.player.fates || [])].filter((id) => typeof id === "string" && id && Number(state.player.suppressedFates?.[id]?.untilTurn || 0) <= turn);
    const fates = ids.map((id) => D().FATE_PATTERNS.find((item) => item.id === id)).filter(Boolean);
    const isCombatTechnique = ["chieu_thuc", "cam_thuat"].includes(technique.category);
    const elementFates = isCombatTechnique ? fates.filter((fate) => technique.element && technique.element !== "vo_he" && technique.element !== "di_he" && fateElement(fate) === technique.element) : [];
    const allowedPaths = Array.isArray(technique.pathAffinity) ? technique.pathAffinity : [];
    const pathFates = isCombatTechnique && pathId && allowedPaths.includes(pathId) ? fates.filter((fate) => {
      const affinity = fatePathAffinity(fate);
      return affinity.lead.includes(pathId) || affinity.support.includes(pathId);
    }) : [];
    const policy = window.CONG_PHAP_DATA?.resonancePolicy || { perMatchingFatePct: 1, totalCapPct: 5 };
    const bonusPct = Math.min(Number(policy.totalCapPct || 5), (elementFates.length + pathFates.filter((fate) => !elementFates.some((entry) => entry.id === fate.id)).length) * Number(policy.perMatchingFatePct || 1));
    return { bonusPct, elementFateIds: elementFates.map((fate) => fate.id), pathFateIds: pathFates.map((fate) => fate.id), pathId, techniquePathAffinity: allowedPaths.slice() };
  }
  function techniqueCombatProjection(state, technique, progress, stance = "steady", enemy = null, statsOverride = null, corruptionOverride = null) {
    const stats = statsOverride || computeStats(state.player), visible = technique.visibleStats || {}, effect = technique.effect || {};
    const corruptionRating = corruptionOverride == null ? Number(state.player.corruptionRating || 0) : Number(corruptionOverride);
    const evolution = window.GameExpansion?.techniqueEvolutionModifiers ? window.GameExpansion.techniqueEvolutionModifiers(state, technique.id) : {};
    const mastery = (window.CONG_PHAP_DATA?.masteryMultipliers || [0.6, 0.8, 1, 1.15, 1.3])[progress.masteryStage || 0];
    const core = activeCoreMethod(state), relation = core ? elementRelation(core.element, technique.element) : "neutral";
    ...
    const family = technique.family || "thuong"; let familyMult = enemy ? familyMatchup(family, enemy.family || "thuong") : 1;
    ...
    const guildCombat = window.GameExpansion?.guildTechniqueCombatBonus?.(state, technique) || { powerPct: 0, sourceIds: [] };
  ...
  function techniquePreview(state, id, options = {}) {
    ...
    const previewEnemyId = options.targetId || Object.keys(state.enemies || {})[0];
    const previewEnemy = previewEnemyId && combatEntity(state, previewEnemyId);
    const combatPreview = previewEnemy && ["chieu_thuc", "cam_thuat"].includes(technique.category) ? techniqueCombatProjection(state, technique, progress, stance, previewEnemy, stats, clamp(Number(state.player.corruptionRating || 0) + corruptionCost, 0, 100)) : null;
    const resourceBlockers = [];
    const fateResonance = techniqueFateResonance(state, technique);
    const guildCombatPreview = window.GameExpansion?.guildTechniqueCombatBonus?.(state, technique) || { powerPct: 0, sourceIds: [] };
    ...
    return {
      ...
      resourcesReady: resourceBlockers.length === 0, blockers: resourceBlockers,
      costs: { manaCost, staminaCost, sanCost, lifespanCost, corruptionCost },
      prepareCost,
      combatState: copy(combatState),
      prepared: Boolean(combatState.prepared),
      worldModifiers,
      stance,
  ...
  function useTechnique(state, id, options = {}) {
    ...
    const uiSequence = /^technique-ui:(\d+)$/.exec(actionKey);
    state.player.techniqueActionReceipts ||= {};
    if (uiSequence && Number(uiSequence[1]) <= Number(state.player.techniqueActionReceiptHighWater || 0) && !state.player.techniqueActionReceipts[actionKey]) return { success: false, duplicate: true, reason: "Action thi triển cũ đã được xử lý; không thể phát lại." };
    const priorReceipt = state.player.techniqueActionReceipts[actionKey];
    if (priorReceipt) { if (priorReceipt.techniqueId !== id || priorReceipt.stance !== stance) return { success: false, duplicate: true, reason: "Action ID đã được dùng cho một thao tác khác." }; const receipt = copy(priorReceipt); return { success: receipt.success !== false, duplicate: true, receipt }; }
    if (options.requirePrepared || combatState.prepared) {
    ...
    const actionReceipt = state.player.techniqueActionReceipts[actionKey] = { actionId: actionKey, techniqueId: id, stance, status: "committed", turn: Number(state.meta?.turn || 0), costs: { manaCost, staminaCost, sanCost, lifespanCost, corruptionCost } };
    if (uiSequence) state.player.techniqueActionReceiptHighWater = Number(uiSequence[1]);
    const receiptKeys = Object.keys(state.player.techniqueActionReceipts);
    if (receiptKeys.length > 64) delete state.player.techniqueActionReceipts[receiptKeys[0]];
    ...
    if (state.player.san <= 0) {
      ...
      return { success: false, committed: true, actionId: actionKey, reason: "Thanh Tỉnh đã cạn; hình phạt Mất Trí được kích hoạt." };
    }
    ...
    if (technique.category === "than_phap") {
      ...
      state.flags.activeFormation = { techniqueId: id, sourceGuildId: technique.sourceGuildId || null, untilTurn: state.meta.turn + Math.max(1, cooldown) };
      pushHistory(state, { type: "sys", text: "§ Đã triển khai trận pháp " + technique.name + "." });
    } else {
      if (!enemy) {
        updateDerived(state); actionReceipt.success = true; actionReceipt.outcome = "no_target";
        return { success: true, reason: "Công pháp được vận chuyển nhưng không có mục tiêu." };
      }
      const combatProjection = techniqueCombatProjection(state, technique, progress, stance, enemy, stats, Number(state.player.corruptionRating || 0));
      const damage = Math.max(1, Math.round(combatProjection.rawDamage + replayInt(state, "technique-damage:" + id + ":" + Number(state.meta?.turn || 0), 0, 6)));
      if (isOffensive && enemy) updateTechniqueMastery(state, technique, progress, "combat");
      actionReceipt.success = true; actionReceipt.outcome = "hit"; actionReceipt.damage = damage;
      ...
      return { success: true, stance, actionId: actionKey };
    }
    afterPlayerCombatAction(state);
    actionReceipt.success = true; actionReceipt.outcome ||= "resolved";
    return { success: true, stance, actionId: actionKey };
  }
  ...
  function techniqueStatus(state) {
    return getKnownTechniques(state).map((t) => {
      ...
      return t.name + " · " + t.element + " · " + stages[p.masteryStage || 0] + " · " + (p.masteryExp || 0) + " kinh nghiệm lĩnh ngộ" + (hidden.length ? " · ẩn tính: " + hidden.map((item) => item.attribute).join(", ") : "");
    }).join("\n");
  }
  ...
  function createState(input) {
    ...
    const state = {
      meta: {
        saveId: "save_" + String(character.id || character.name || "player") + "_" + String(input.worldId || "co_di_dien") + "_" + String(startLocation),
  ...
  function refreshMarket(state, day = gameDayIndex(ensureGameClock(state))) {
    state.market = state.market || { generatedDay: 0, refreshIntervalDays: 1, offers: [], purchased: {} };
    const generatedDay = Number(state.market.generatedDay ?? state.market.generatedAt ?? 0);
    if (state.market.offers.length && Number(day) - generatedDay < Number(state.market.refreshIntervalDays || 1)) return state.market;
    ...
    state.market.generatedDay = Number(day); state.market.generatedAt = Number(day); state.market.offers = [...fates, ...items]; state.market.purchased = {};
    return state.market;
  }

  function marketOffers(state, day = gameDayIndex(ensureGameClock(state))) { return refreshMarket(state, day).offers; }
  ...
  /* ---------- Inventory ---------- */
  function itemDefinition(state, itemId) {
    return state?.generatedItems?.[itemId] || D().ITEMS?.[itemId] || null;
  }

  function ensureTechniqueCombatState(progress) {
    progress.combatState ||= {};
    const combatState = progress.combatState;
    combatState.cooldownRemaining = Math.max(0, Number(combatState.cooldownRemaining || 0));
    combatState.channelProgress = clamp(Number(combatState.channelProgress || 0), 0, 1);
    combatState.prepared = Boolean(combatState.prepared);
    combatState.preparedActionId = combatState.preparedActionId || null;
    combatState.preparedTurn = combatState.preparedTurn == null ? null : Number(combatState.preparedTurn);
    combatState.lastResolvedActionId = combatState.lastResolvedActionId || null;
    return combatState;
  }

  function resolveTechniquePrepareCost(technique, resolvedCosts) {
    const declared = technique.prepareCost || technique.cost?.prepare || {};
    const mana = declared.mana ?? declared.qi;
    if (mana != null) return { manaCost: Math.max(0, Number(mana)), staminaCost: Math.max(0, Number(declared.stamina || 0)) };
    return { manaCost: 0, staminaCost: 0 };
  }

  function prepareTechnique(state, id, actionId, options = {}) {
    const technique = techniqueCatalog()[id];
    const progress = technique && state.player.techniques?.[id] && (state.player.techniques[id] = normalizeTechniqueProgress(state.player.techniques[id]));
    if (!technique || !progress) return { success: false, reason: "Chưa lĩnh ngộ Công pháp này." };
    if (!actionId || typeof actionId !== "string") return { success: false, reason: "Thiếu Action ID chuẩn cho bước chuẩn bị." };
    if (technique.category === "tam_phap") return { success: false, reason: "Tâm Pháp không có bước chuẩn bị thi triển." };
    const combatState = ensureTechniqueCombatState(progress);
    state.player.techniquePrepareReceipts ||= {};
    const previous = state.player.techniquePrepareReceipts[actionId];
    if (previous) return { success: previous.success !== false, duplicate: true, receipt: copy(previous) };
    const preview = techniquePreview(state, id, { stance: options.stance || "steady", targetId: options.targetId });
    ...
    if (Number(state.player.qi || 0) < prepareCost.manaCost) return { success: false, reason: "Linh khí không đủ để chuẩn bị." };
    if (Number(state.player.stamina || 0) < prepareCost.staminaCost) return { success: false, reason: "Thể lực không đủ để chuẩn bị." };
    state.player.qi -= prepareCost.manaCost;
    state.player.stamina -= prepareCost.staminaCost;
    combatState.prepared = true;
    combatState.preparedActionId = actionId;
    combatState.preparedTurn = currentTurn;
    combatState.channelProgress = 0;
    const receipt = state.player.techniquePrepareReceipts[actionId] = { actionId, techniqueId: id, status: "prepared", turn: currentTurn, cost: copy(prepareCost) };
    Object.keys(state.player.techniquePrepareReceipts).slice(0, -64).forEach((key) => delete state.player.techniquePrepareReceipts[key]);
    return { success: true, actionId, techniqueId: id, prepareCost: copy(prepareCost), channelProgress: 0, receipt: copy(receipt) };
  }

  function advanceTechniqueChannel(state, id, delta = 1) {
    const progress = state.player.techniques?.[id] && (state.player.techniques[id] = normalizeTechniqueProgress(state.player.techniques[id]));
    if (!progress) return { success: false, reason: "Chưa lĩnh ngộ Công pháp này." };
    const combatState = ensureTechniqueCombatState(progress);
    if (!combatState.prepared) return { success: false, reason: "Công pháp chưa được chuẩn bị." };
    combatState.channelProgress = clamp(combatState.channelProgress + Math.max(0, Number(delta) || 0), 0, 1);
    return { success: true, techniqueId: id, channelProgress: combatState.channelProgress, ready: combatState.channelProgress >= 1 };
  }

  function cancelTechniquePreparation(state, id, reason = "cancelled") {
    const progress = state.player.techniques?.[id] && (state.player.techniques[id] = normalizeTechniqueProgress(state.player.techniques[id]));
    if (!progress) return { success: false, reason: "Chưa lĩnh ngộ Công pháp này." };
    const combatState = ensureTechniqueCombatState(progress);
    if (!combatState.prepared) return { success: false, reason: "Công pháp chưa được chuẩn bị." };
  ...
  function addItem(state, itemId, qty = 1) {
    if (!itemDefinition(state, itemId)) return false;
  ...
  function removeItem(state, itemId, qty = 1) {
    ...
    state.inventory[itemId] = available - quantity;
  ...
  function fateVaultCapacity(state) {
    ...
    state.flags.fateSlotCapacity = capacity;
    return capacity;
  }
  ...
  function pathRelation(pathId) {
    ...
    return (typeof window !== "undefined" && window.PATH_FATE_RELATIONS?.paths?.[canonical]) || null;
  }
  function selectPath(state, pathId) {
    ...
    if (pathId === "ngoai_dao_gia") {
      ...
      return { success: true, pathId, namespace: "unbound", score: 0 };
    }
  ...
  function pathMatchSummary(character, pathId) {
    if (pathId === "ngoai_dao_gia" || character?.unbound || character?.pathNamespace === "unbound") return { score: 0, lead: 0, support: 0, forbidden: 0, daoBonus: 0, allHung: false, namespace: "unbound" };
    const relation = pathRelation(pathId);
    ...
    const daoBonus = Number(character.daoTam || 0) >= 80 ? 1 : 0;
  ...
  function mergeFates(state, fateIds) {
    ...
    const relationships = (typeof window !== "undefined" && window.FATE_RELATIONSHIPS) || {};
    const key = (value) => String(value?.id || value);
    const selected = ids.slice().sort();
    const recipe = (relationships.fusion_recipes || []).find((entry) => {
      const materials = (entry.materials || []).map(key).sort();
      return materials.length === selected.length && materials.every((id, index) => id === selected[index]);
    });
    if (!recipe?.result?.id) return { success: false, reason: "Bộ Mệnh đã chọn không khớp công thức dung hợp canonical." };
    const fate = D().FATE_PATTERNS.find((item) => item.id === recipe.result.id);
    if (!fate) return { success: false, reason: "Kết quả công thức không tồn tại trong catalog Mệnh canonical.", recipeId: recipe.recipe_id };
    const successRate = clamp(Number(recipe.success_rate_pct ?? 100) / 100, 0, 1);
    ids.forEach((id) => vault.splice(vault.indexOf(id), 1));
    const roll = replayRandom(state, "fate-fusion:" + String(recipe.recipe_id) + ":" + Number(state.meta?.turn || 0));
    if (roll > successRate) {
      if (/SAN\s*-\s*5/i.test(String(recipe.fail_consequence || ""))) drainSan(state, 5, "phản phệ dung hợp Mệnh");
      pushHistory(state, { type: "warn", text: "Dung hợp Mệnh thất bại; các nguyên liệu đã tan vào phản phệ." });
      return { success: false, failed: true, recipeId: recipe.recipe_id, roll, successRate };
    }
    const result = receiveFate(state, fate.id, { source: "fusion_recipe:" + recipe.recipe_id, allowPending: true });
    if (!result?.added && !result?.pending) return { success: false, reason: result?.reason || "Không thể nhận kết quả dung hợp.", recipeId: recipe.recipe_id };
    pushHistory(state, { type: "sys", text: "Mệnh Kho dung hợp theo công thức " + recipe.recipe_id + ", sinh ra " + fate.name + "." });
    return { success: true, recipeId: recipe.recipe_id, fate, result, roll, successRate };
  }
  function getComboEligibility(state, fateIds = []) {
    const ids = new Set(fateIds);
    const combos = (D().FATE_RELATIONSHIPS?.combo_sets || []).filter((combo) => (combo.members || []).every((member) => ids.has(member.id)));
    return { eligible: combos.length > 0, combos, missing: (D().FATE_RELATIONSHIPS?.combo_sets || []).filter((combo) => !(combo.members || []).every((member) => ids.has(member.id))).map((combo) => ({ id: combo.id, missing: (combo.members || []).map((member) => member.id).filter((id) => !ids.has(id)) })) };
  }
  function fuseFate(state, fateIds, options = {}) { return mergeFates(state, fateIds, options); }
  function chooseDuplicateResolution(state, fateId, resolution = "vault") {
    const pending = state.pendingFateReward;
    if (!pending || pending.fateId !== fateId) return { success: false, reason: "Không có Mệnh Số trùng đang chờ xử lý." };
    if (resolution === "discard") return { success: dismissPendingFateReward(state), resolution };
    return { ...resolvePendingFateReward(state, resolution === "vault" ? null : resolution), resolution };
  }
  function serverUniqueFateOwnership(state, fateId) { return fateUniqueOwner(state, D().FATE_PATTERNS.find((fate) => fate.id === fateId)); }
  ...
  function registerGeneratedItem(state, item) {
    ...
    state.player.generatedItems = state.player.generatedItems || {};
    state.player.generatedItems[item.id] = state.generatedItems[item.id];
    return true;
  }
  ...
  function chooseOrigin(state, branch, specializationId) {
    ...
    state.flags.originChoicePending = false;
    state.flags.originChoice = { branch, specialization: specializationId, chosenAt: gameDayIndex(ensureGameClock(state)), consequences: [], confirmed: true };
    state.flags.originLocked = true;
  ...
  function questDefinition(state, questId) {
    return state?.questDefinitions?.[questId] || D().QUESTS?.[questId] || null;
  }

  function activateQuest(state, questId) {
    const q = state.quests[questId];
    if (q && (q.status === "available" || q.status === "locked")) {
      q.status = "active"; q.discoveredAt = q.discoveredAt || gameDayIndex(ensureGameClock(state)); q.tracked = true;
    }
  }
  ...
  function abandonQuest(state, questId, reason = "Người chơi từ bỏ") {
    const q = state.quests?.[questId]; if (!q || !["active", "available"].includes(q.status)) return false;
      q.status = "failed"; q.failedAt = gameDayIndex(ensureGameClock(state)); q.failureReason = reason; pushHistory(state, { type: "warn", text: "× Đã từ bỏ nhiệm vụ: " + (q.title || questId) + "." }); return true;
  }
  function maybeSpawnDynamicQuest(state) {
    ...
    const loc = runtimeLocationPool(state)[state.locationId]; if (!loc) return;
    const id = "dynamic_trace_" + state.meta.turn;
    const title = "Dấu Chân Chưa Khép · " + loc.name;
    state.questDefinitions ||= {};
    state.questDefinitions[id] = { id, title, kind: "co_duyen", priority: 2, objectives: [{ id: "observe", label: "Quan Sát hoặc Tìm Kiếm tại khu vực này", check: (st) => Number(st.flags.searches?.[st.locationId] || 0) > 0 || st.flags.lastCombatOutcome === "fled" }], reward: { exp: 25, merit: 3 } };
    state.quests[id] = { id, type: "co_duyen", title, priority: 2, status: "active", tracked: true, discoveredAt: gameDayIndex(ensureGameClock(state)), objectives: [{ id: "observe", label: "Quan Sát hoặc Tìm Kiếm tại khu vực này", done: false }], branches: [{ id: "investigate", label: "Điều tra dấu vết", alternatives: ["search", "talk"] }], alternatives: ["search", "talk", "combat"], consequences: {}, expiresDay: gameDayIndex(ensureGameClock(state)) + 1 };
  ...
  function checkQuestObjectives(state, questId) {
    ...
    const def = questDefinition(state, questId);
    let allDone = true;
    ...
    if (allDone) {
      q.status = "completed";
      q.completedAt = gameDayIndex(ensureGameClock(state));
      const r = def.reward || {};
      ...
      const canonicalReward = typeof window !== "undefined" && window.GameExpansion?.grantCanonicalReward
        ...
        : null;
      if (!canonicalReward) {
        if (r.exp) gainExp(state, r.exp, "quest_reward", { sourceId: "quest" });
    ...
    if (r.item) summary.rewards.push({ type: "Vật phẩm", value: itemDefinition(state, r.item)?.name || r.item });
  ...
  function failQuest(state, questId, reason) {
    ...
    if (q && q.status === "active") {
      q.status = "failed"; q.failedAt = gameDayIndex(ensureGameClock(state)); q.failureReason = reason || "Không hoàn thành mục tiêu";
    pushHistory(state, { type: "warn", text: "× Nhiệm vụ thất bại: " + (questDefinition(state, questId)?.title || questId) + (reason ? " (" + reason + ")" : "") });
    }
  }
  ...
  function safeTravelDestination(state) {
    const hubs = travelHubCatalog().filter((hub) => runtimeLocationPool(state)[hub.id]);
    const home = state.homeLocationId && runtimeLocationPool(state)[state.homeLocationId] ? state.homeLocationId : (state.startLocationId && runtimeLocationPool(state)[state.startLocationId] ? state.startLocationId : null);
  ...
  function joinGuild(state, query) {
    ...
    const guildNodeId = guildAddress?.nodeId || Object.keys(runtimeLocationPool(state) || {}).find((nodeId) => runtimeLocationPool(state)[nodeId]?.organizationId === guild.id) || null;
    if (!guildNodeId || !runtimeLocationPool(state)[guildNodeId]) {
    ...
    state.guildMembership = {
      ...
      rankId: "outer",
      rankIndex: 0,
      revision: Number(state.guildMembershipRevision || 0) + 1,
  ...
  function recordCultivationGain(state, amount, source = "system_other", metadata = {}) {
    ...
    const gainKey = day + ":" + source + ":" + Number(state.meta?.turn || 0) + ":" + state.player.cultivation.velocitySamples.length;
    if (deviationChance && replayRandom(state, "cultivation-deviation:" + gainKey) < deviationChance) {
      const severe = replayRandom(state, "cultivation-deviation-severity:" + gainKey) < clamp(0.20 + Number(state.player.corruptionRating || 0) / 200, 0.20, 0.70);
      ...
      else { loss = Math.ceil(state.player.exp * (replayInt(state, "cultivation-deviation-loss:" + gainKey, 10, 20) / 100)); state.player.exp = Math.max(0, state.player.exp - loss); state.player.madnessPoints = Number(state.player.madnessPoints || 0) + 5; }
  ...
  function anchorCandidates(state) {
    ...
    return Object.entries(relationships).map(([npcId, relation]) => {
      ...
      const suspicion = Number(relation?.suspicion || 0), fear = Number(relation?.fear || 0);
      const npcState = typeof window !== "undefined" ? window.GameExpansion?.npcWorldContext?.(state, npcId) : null;
      const npcClass = npcState?.classification || relation?.npcClass || "persistent_named";
      if (!(trust >= 55 && respect >= 35 && suspicion <= 35 && fear <= 50)) return null;
      if (!["persistent_named", "bondable_encounter"].includes(npcClass)) return null;
      const meaningful = Array.isArray(state.relationshipEvents?.[npcId]) && state.relationshipEvents[npcId].some((event) => ["saved", "kept_promise", "shared_reward", "supported_faction"].includes(event.tag));
      if (!meaningful && !relation.identityRemembered) return null;
      return { npcId, name: npc.name || npcId, type: "npc", source: "npc_relationship", trust, respect, suspicion, fear, stability: clamp(Math.round(35 + trust * 0.55 + respect * 0.35 - suspicion * 0.2 - fear * 0.1), 25, 100) };
  ...
  function establishHumanAnchor(state, npcId) {
    ...
    const active = state.player.anchors.filter((anchor) => anchor.status !== "broken" && !anchor.broken && Number(anchor.stability || 0) > 0);
    if (active.length >= 3) return { success: false, reason: "Đã đạt tối đa 3 Neo Nhân Tính." };
    const anchor = { id: "anchor_npc_" + npcId, name: "Neo Nhân Tính · " + candidate.name, type: "npc", source: "npc_relationship", npcId, stability: candidate.stability, maxStability: candidate.stability, status: "active", lastNurturedAt: Number(state.meta?.turn || 0), integrity: "intact", broken: false, establishedAtTurn: Number(state.meta?.turn || 0) };
  ...
  function performBreakthroughRitualStep(state, step) {
    ...
    flags.attemptLog.push({ gate: step, timestamp: gameDayIndex(ensureGameClock(state)), result: "fail", sanCost: sanCost || null });
    ...
    flags.attemptLog = Array.isArray(flags.attemptLog) ? flags.attemptLog : [];
    flags.attemptLog.push({ gate: step, timestamp: gameDayIndex(ensureGameClock(state)), result: "pass", sanCost: step === "cost" ? 5 : null });
  ...
  function suggestFateForRealmRequirement(character, requirementType) {
    ...
    const candidates = D().FATE_PATTERNS.filter((fate) => !owned.has(fate.id)).map((fate) => {
      ...
      const rarity = { phan: 1, linh: 2, hoang: 3, huyen: 4, dia: 5, thien: 6, thanh: 7, tien: 8 }[fate.grade] || 1;
  ...
  const FATE_REWARD_WEIGHTS = Object.freeze({
    ...
    "9+": { phan: 1, linh: 5, hoang: 15, huyen: 25, dia: 25, thien: 18, thanh: 11, tien: 0 }
  });
  ...
  function rollFateByProgression(state, options = {}) {
    ...
    const pool = D().FATE_PATTERNS.filter((fate) => {
      ...
      if (Array.isArray(options.pathAffinity) && options.pathAffinity.length && !options.pathAffinity.some((tag) => fateTags(fate).includes(normalizedText(tag)))) return false;
      return true;
    });
    ...
    if (gradeWeights.length) {
      ...
      const pityKey = options.pityKey ? String(options.pityKey) : null;
  ...
  function cultivate(state, options = {}) {
    ...
    if (state.player.qi < qiCost) {
      if (!options.silent) pushHistory(state, { type: "warn", text: "× Linh khí không đủ để tu luyện (cần " + qiCost + ")." });
      return;
    }
    ...
    if (state.guildMembership) {
      ...
      if (state.guildMembership.rank !== oldRank) {
        if (!options.silent) pushHistory(state, { type: "sys", text: "§ Thân phận tông môn thăng lên " + state.guildMembership.rank + "." });
      }
      grantGuildTechniques(state);
    }
    gainExp(state, exp, "cultivate");
    let masteryTotal = 0;
  ...
  // Canonical persisted secluded-cultivation session. Progress is advanced in
  // game days, so reload/offline processing cannot silently restart the work.
  function startSecludedCultivation(state, days = 1) {
    const totalDays = clamp(Math.floor(Number(days) || 1), 1, 30);
    if (state.player.secludedSession?.status === "active") return { success: false, reason: "Đã có một phiên bế quan đang diễn ra." };
    if (aliveEnemies(state).length || state.pendingEnding) return { success: false, reason: "Không thể bế quan khi đang giao chiến hoặc hành trình đã kết thúc." };
    state.player.secludedSession = { id: "secluded:" + Number(state.meta?.turn || 0) + ":" + cultivationDay(state), status: "active", startedDay: cultivationDay(state), totalDays, completedDays: 0, locationId: state.locationId, result: null };
    return advanceSecludedCultivation(state, totalDays);
  }
  function advanceSecludedCultivation(state, days = 1) {
    const session = state.player.secludedSession;
    if (!session || session.status !== "active") return { success: false, reason: "Không có phiên bế quan đang diễn ra." };
    const remaining = Math.max(0, session.totalDays - session.completedDays);
    const limit = clamp(Math.floor(Number(days) || 1), 1, remaining || 1);
    let cultivated = 0;
    for (let index = 0; index < limit; index += 1) {
      const result = autoCultivate(state, 6);
      cultivated += Number(result.completed || 0);
      session.completedDays += 1;
      advanceGameTime(state, 1);
      if (state.pendingEnding || state.flags.madness || breakthroughRequirements(state).ready) break;
    }
    if (session.completedDays >= session.totalDays || state.pendingEnding || state.flags.madness || breakthroughRequirements(state).ready) {
      session.status = "completed";
      session.completedDay = cultivationDay(state);
      session.result = { cultivated, completedDays: session.completedDays };
    }
    return { success: cultivated > 0, completed: cultivated, session: { ...session } };
  }
  ...
  const OPEN_WORLD_BOUNDS = Object.freeze({ min: 0, max: 100 });
  function coordinateKey(x, y) { return Math.floor(Number(x)) + "," + Math.floor(Number(y)); }
  function neighborCoordinate(x, y, direction) {
    const delta = OPEN_WORLD_DELTAS[direction];
    if (!delta) return null;
    return { x: Math.floor(Number(x)) + delta[0], y: Math.floor(Number(y)) + delta[1] };
  }
  ...
  function ensureOpenWorld(state) {
    ...
    // Canonical nodes are part of the same Oxy grid as procedural nodes. Older
    // saves only indexed the current node, which made a later move create a
    // duplicate node at an already occupied coordinate.
    Object.entries(D().WORLD_MAP?.locations || {}).forEach(([id, point]) => {
      if (!runtimeLocationPool(state)[id]) return;
      if (!Array.isArray(state.openWorld.coordinates[id]) && Number.isFinite(Number(point?.x)) && Number.isFinite(Number(point?.y))) state.openWorld.coordinates[id] = [Number(point.x), Number(point.y)];
      const coordinates = state.openWorld.coordinates[id];
    ...
    Object.entries(state.openWorld.nodes).forEach(([id, node]) => {
      ...
      runtimeLocationPool(state)[id] = node;
    ...
    Object.entries(state.openWorld.exits).forEach(([fromId, exits]) => {
      const from = state.openWorld.coordinates[fromId];
      if (!from) return;
    ...
    Object.entries(state.openWorld.exits).forEach(([from, exits]) => {
      Object.entries(exits).forEach(([dir, to]) => {
        ...
        const knownReverse = state.openWorld.exits[to]?.[reverse] || runtimeLocationPool(state)[to]?.exits?.[reverse];
  ...
  function locationExits(state, locationId = state.locationId) {
    const world = ensureOpenWorld(state), coordinates = nodeCoordinates(state, locationId);
    if (!coordinates) return { ...(runtimeLocationPool(state)[locationId]?.exits || {}), ...(world.exits[locationId] || {}) };
    const exits = {};
    Object.keys(OPEN_WORLD_DELTAS).forEach((direction) => {
      const target = neighborCoordinate(coordinates.x, coordinates.y, direction);
      exits[direction] = target && target.x >= OPEN_WORLD_BOUNDS.min && target.x <= OPEN_WORLD_BOUNDS.max && target.y >= OPEN_WORLD_BOUNDS.min && target.y <= OPEN_WORLD_BOUNDS.max
        ? getNodeAtCoordinate(state, target.x, target.y)
        : null;
    });
  ...
  function generateOpenWorldNode(state, x, y) {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y) || x < OPEN_WORLD_BOUNDS.min || x > OPEN_WORLD_BOUNDS.max || y < OPEN_WORLD_BOUNDS.min || y > OPEN_WORLD_BOUNDS.max) return null;
    const world = ensureOpenWorld(state);
    const id = "open_" + x + "_" + y;
    const coordinateKey = x + "," + y;
    const existingId = world.coordinateIndex[coordinateKey];
    if (existingId && runtimeLocationPool(state)[existingId]) return existingId;
    if (world.nodes[id]) { runtimeLocationPool(state)[id] = world.nodes[id]; world.coordinates[id] = [x, y]; world.coordinateIndex[coordinateKey] = id; return id; }
    const hash = openWorldHash(x, y);
    const distance = Math.abs(x) + Math.abs(y);
    const regionAnchors = (D().WORLD_MAP?.regions || []).map((entry) => ({ entry, dx: Number(entry.x || 50) - x, dy: Number(entry.y || 50) - y }));
    const region = regionAnchors.sort((a, b) => (a.dx * a.dx + a.dy * a.dy) - (b.dx * b.dx + b.dy * b.dy))[0]?.entry?.id || "trung_vuc";
  ...
  function validateOpenWorldGrid(state) {
    ...
    Object.entries(world.coordinates || {}).forEach(([nodeId, coordinates]) => {
      const runtimeHiddenNode = Boolean(runtimeLocationPool(state)[nodeId]?.runtime && runtimeLocationPool(state)[nodeId]?.hiddenRealm);
      if (!Array.isArray(coordinates) || coordinates.length < 2 || (!runtimeHiddenNode && (!Number.isInteger(Number(coordinates[0])) || !Number.isInteger(Number(coordinates[1]))))) { errors.push(nodeId + ":coordinate"); return; }
      const x = Number(coordinates[0]), y = Number(coordinates[1]), key = x + "," + y;
      if (x < OPEN_WORLD_BOUNDS.min || x > OPEN_WORLD_BOUNDS.max || y < OPEN_WORLD_BOUNDS.min || y > OPEN_WORLD_BOUNDS.max) errors.push(nodeId + ":bounds");
      if (seen.has(key) && seen.get(key) !== nodeId) errors.push(key + ":duplicate"); else seen.set(key, nodeId);
      if (!runtimeLocationPool(state)[nodeId]) errors.push(nodeId + ":missing-location");
    });
    Object.entries(world.nodes || {}).forEach(([nodeId, node]) => { if (!node?.openWorld || !node?.exits || Object.keys(OPEN_WORLD_DELTAS).some((dir) => !(dir in node.exits))) errors.push(nodeId + ":directions"); });
    Object.entries(world.coordinateIndex || {}).forEach(([key, nodeId]) => {
      const coordinates = world.coordinates[nodeId];
      if (!coordinates || coordinateKey(coordinates[0], coordinates[1]) !== key) errors.push(key + ":index-mismatch");
    });
    Object.entries(world.exits || {}).forEach(([fromId, exits]) => {
      const from = world.coordinates[fromId];
      if (!from) return;
      Object.entries(exits || {}).forEach(([direction, toId]) => {
        if (!toId) return;
    ...
    return { ok: errors.length === 0, errors, bounds: { ...OPEN_WORLD_BOUNDS }, nodeCount: Object.keys(world.coordinates || {}).length };
  }
  function move(state, dir, options = {}) {
    ...
    if (state.pendingMapEvent?.status === "pending" && state.pendingMapEvent.nodeId !== target) {
      const abandoned = state.pendingMapEvent, record = mapEventRecord(state, abandoned.nodeId);
      record.cooldownUntilTurn = Number(state.meta?.turn || 0) + 5;
      record.resolvedIds.push(abandoned.eventId);
      state.mapEvents.history.push({ ...abandoned, status: "abandoned", resolvedTurn: Number(state.meta?.turn || 0) });
      state.mapEvents.history = state.mapEvents.history.slice(-40);
      state.pendingMapEvent = null;
      pushHistory(state, { type: "narr", text: "Khi rời khỏi " + (runtimeLocationPool(state)[abandoned.nodeId]?.name || "khu vực ấy") + ", ngươi bỏ lại một phát hiện ẩn; dấu vết mờ dần trong gió." });
    }
    ...
    const newLoc = runtimeLocationPool(state)[target];
    ...
    maybeTriggerRandomEncounter(state, firstDiscovery ? "first_discovery" : "moving_through");
  ...
  function confirmPendingDeparture(state) {
    ...
    if (opportunity) {
      ...
      opportunity.choice = "abandoned";
      opportunity.resolvedDay = absoluteDay(state.gameClock);
      if (typeof window !== "undefined" && window.GameExpansion?.recordContestedOpportunity) window.GameExpansion.recordContestedOpportunity(state, opportunity);
      state.pendingContestedOpportunity = null;
    }
  ...
  function searchStatus(state, locationId = state.locationId) {
    const loc = runtimeLocationPool(state)[locationId];
  ...
  function ensureSearchChainQuest(state, locationId, requiredSearches) {
    ...
    if (!state.quests[id]) state.quests[id] = { id, type: "co_duyen", title: state.questDefinitions[id].title, priority: 2, status: "active", tracked: true, discoveredAt: gameDayIndex(ensureGameClock(state)), searchLocationId: locationId, requiredSearches: Number(requiredSearches || 1), rewardFateId: fateReward?.id || null, objectives: [{ id: "follow_trace", label: "Tìm kiếm thêm để lần theo dấu vết", done: false }], branches: [], alternatives: ["search"], consequences: {}, expiresDay: gameDayIndex(ensureGameClock(state)) + 7 };
    return id;
  }
  function search(state) {
    ...
    if (!status.searchable) {
      ...
      return { success: false, reason: "Độ sâu dò tìm đã cạn." };
    }
    const staminaCost = 5;
    ...
    Object.entries(resources).forEach(([itemId, qty]) => findings.push({ type: "resource", itemId, qty, label: (itemDefinition(state, itemId)?.name || itemId) + " ×" + qty }));
  ...
  function investigateSearchFinding(state) {
    ...
    if (site.chainStage === 2 && !site.secretLocationId) {
      ...
      if (secretDirection) {
        ...
        const secret = runtimeLocationPool(state)[secretId];
        if (secret?.openWorld) {
          secret.name = "Động Phủ Bị Xóa Tên";
    ...
    pushHistory(state, { type: "sys", text: "Mảnh ngọc giản hé lộ một lời nhờ cậy chưa ai dám nhận — " + (questDefinition(state, questId)?.title || questId) + "." });
    }
  ...
  function useItem(state, name) {
    const itemId = Object.keys(state.inventory).find((id) => {
      const it = itemDefinition(state, id);
    ...
    const it = itemDefinition(state, itemId);
    ...
    if (it.exp > 0) gainExp(state, it.exp, "item_use", { itemId });
  ...
  function regionOfLocation(state) {
    return D().WORLD_MAP?.locations?.[state.locationId]?.region || runtimeLocationPool(state)?.[state.locationId]?.region || null;
  }
  function mapAddressCatalog() {
    const addresses = D().WORLD_MAP?.addresses || {};
    return {
      services: Array.isArray(addresses.services) ? addresses.services : [],
      spawnPoints: Array.isArray(addresses.spawnPoints) ? addresses.spawnPoints : [],
      factions: Array.isArray(addresses.factions) ? addresses.factions : [],
      organizations: Array.isArray(addresses.organizations) ? addresses.organizations : []
    };
  }
  function mapAddressesAtNode(nodeId) {
    const catalog = mapAddressCatalog();
    return [...catalog.services, ...catalog.spawnPoints, ...catalog.factions, ...catalog.organizations].filter((address) => address.nodeId === nodeId);
  }
  function mapOxyAddress(refId, kind = "") {
    const catalog = mapAddressCatalog();
    const pool = kind === "faction" ? catalog.factions : kind === "guild" ? catalog.organizations : [...catalog.services, ...catalog.spawnPoints];
    return pool.find((address) => address.id === refId || address.refId === refId || address.nodeId === refId) || null;
  }
  ...
  const MAP_EVENT_GROUP_WEIGHTS = Object.freeze({
    tong_mon_an_toan: { npc: 70, monster: 5, co_duyen: 20, dong_phu: 5 },
    co_dinh_cot_truyen: { npc: 60, monster: 10, co_duyen: 25, dong_phu: 5 },
    linh_vuc: { npc: 40, monster: 30, co_duyen: 25, dong_phu: 5 },
    hoang_da: { npc: 15, monster: 55, co_duyen: 25, dong_phu: 5 },
    bien_thanh: { npc: 50, monster: 20, co_duyen: 20, dong_phu: 10 },
    cam_dia: { npc: 10, monster: 50, co_duyen: 30, dong_phu: 10 },
    vuong_kinh: { npc: 65, monster: 5, co_duyen: 20, dong_phu: 10 },
    hai_vuc_khong_vuc: { npc: 25, monster: 35, co_duyen: 30, dong_phu: 10 }
  });
  ...
  function mapEventPoolTag(state, node = runtimeLocationPool(state)[state.locationId]) {
    ...
    if (Number(node?.dangerLevel || 0) >= 4) return "cam_dia";
    if (Number(node?.dangerLevel || 0) >= 3 || node?.enemies?.length) return "hoang_da";
    if (/thành|cảng|trấn|phường|market/i.test(String(node?.name || ""))) return "bien_thanh";
  ...
  function mapEventRecord(state, nodeId = state.locationId) {
    ...
    state.mapEvents.history ||= [];
    return state.mapEvents.nodes[nodeId] ||= { cooldownUntilTurn: 0, discovered: false, resolvedIds: [] };
  }
  function rollMapEvent(state, trigger = "moving_through") {
    ...
    let cursor = replayRandom(state, "map-event-group:" + state.locationId + ":" + turn) * Math.max(1, total), group = "co_duyen";
    for (const [id, weight] of Object.entries(weights)) { cursor -= Number(weight || 0); if (cursor <= 0) { group = id; break; } }
    if (group === "monster") { record.discovered = true; return null; }
    const candidates = MAP_EVENT_TEMPLATES.filter((event) => event.group === group && !record.resolvedIds.includes(event.id));
    const fallback = MAP_EVENT_TEMPLATES.filter((event) => event.group === "co_duyen" && !record.resolvedIds.includes(event.id));
    ...
    if (event.id === "co_duyen_heavenly_treasure") {
      const catalog = window.EXPANSION_DATA?.heavenlyTreasures || [];
      if (!catalog.length) return null;
      const kind = catalog[Math.floor(replayRandom(state, "treasure-kind:" + pending.id) * catalog.length)];
      const yearRoll = replayRandom(state, "treasure-age:" + pending.id);
      let cursor = yearRoll, yearIndex = 0;
      for (; yearIndex < (kind.yearWeights || []).length - 1; yearIndex += 1) { cursor -= Number(kind.yearWeights[yearIndex] || 0) / 100; if (cursor < 0) break; }
      const years = Number(kind.years?.[yearIndex] || 100);
      const multiplier = Number(kind.tierMultipliers?.[years] || 1);
      pending.treasure = { id: kind.id, name: kind.name, element: kind.element, rarity: kind.rarity, stat: kind.stat, years, amount: Math.round(Number(kind.baseAmount || 0) * multiplier), catalogVersion: Number(window.EXPANSION_DATA?.version || 1) };
      pending.choices[0].label = "Tiếp nhận " + kind.name + " · " + (years >= 10000 ? "Vạn Niên" : years >= 1000 ? "Thiên Niên" : "Bách Niên");
    }
  ...
  function yearsLabel(years) { return Number(years) >= 10000 ? "Vạn Niên" : Number(years) >= 1000 ? "Thiên Niên" : "Bách Niên"; }
  function resolveMapEvent(state, choiceId) {
    const pending = state.pendingMapEvent, event = pending && MAP_EVENT_TEMPLATES.find((entry) => entry.id === pending.eventId);
    if (!pending || pending.status !== "pending" || !event) return { success: false, reason: "Không có phát hiện ẩn nào đang chờ xử lý." };
    if (pending.nodeId !== state.locationId) return { success: false, reason: "Phát hiện đã nằm ngoài tầm với; hãy quay lại đúng node để xử lý." };
    ...
    if (effect === "treasure" && pending.treasure) {
      state.mapEvents.treasureReceipts ||= {};
      if (state.mapEvents.treasureReceipts[pending.id]) { const receipt = state.mapEvents.treasureReceipts[pending.id]; pending.status = "resolved"; pending.choice = choiceId; pending.resolvedTurn = Number(state.meta?.turn || 0); state.pendingMapEvent = null; return { success: true, duplicate: true, receipt }; }
      ...
      rewardText = treasure.name + " · " + yearsLabel(treasure.years) + " · hiệu lực +" + amount + (treasure.stat === "maxQiPct" ? "% Linh Khí tối đa" : "");
      state.mapEvents.treasureReceipts[pending.id] = { eventInstanceId: pending.id, treasureId: treasure.id, years: treasure.years, amount, stat: treasure.stat, claimedTurn: Number(state.meta?.turn || 0) };
    }
  ...
  function validateHeavenlyTreasureCatalog() {
    ...
    catalog.forEach((entry) => {
      if (!entry?.id || ids.has(entry.id)) errors.push("identity:" + entry?.id); ids.add(entry?.id);
      if (!entry?.name || !allowedStats.has(entry?.stat)) errors.push("effect:" + entry?.id);
      if (!Number.isFinite(Number(entry?.baseAmount)) || Number(entry.baseAmount) <= 0) errors.push("amount:" + entry?.id);
      if (!Array.isArray(entry?.years) || !Array.isArray(entry?.yearWeights) || entry.years.length !== entry.yearWeights.length || entry.years.some((value) => !Number.isFinite(Number(value)) || Number(value) <= 0) || Math.abs(entry.yearWeights.reduce((sum, value) => sum + Number(value || 0), 0) - 100) > 0.001) errors.push("years:" + entry?.id);
      entry?.yearWeights?.forEach((weight) => { if (!Number.isFinite(Number(weight)) || Number(weight) < 0) errors.push("weight:" + entry.id); });
      entry?.years?.forEach((year) => { if (!Number.isFinite(Number(entry.tierMultipliers?.[year])) || Number(entry.tierMultipliers[year]) <= 0) errors.push("multiplier:" + entry.id + ":" + year); });
    });
    if (!catalog.length) errors.push("empty");
    return { ok: errors.length === 0, count: catalog.length, errors };
  }
  function validateMapEventState(state) {
    const runtime = state?.mapEvents || { nodes: {}, history: [] }, errors = [];
    if (!runtime.nodes || typeof runtime.nodes !== "object") errors.push("nodes");
    if (!Array.isArray(runtime.history)) errors.push("history");
    const treasureCatalogAudit = validateHeavenlyTreasureCatalog();
    if (!treasureCatalogAudit.ok) treasureCatalogAudit.errors.forEach((error) => errors.push("treasureCatalog:" + error));
    Object.entries(runtime.treasureReceipts || {}).forEach(([key, receipt]) => { if (receipt?.eventInstanceId !== key || !window.EXPANSION_DATA?.heavenlyTreasures?.some((entry) => entry.id === receipt.treasureId) || !Number.isFinite(Number(receipt.amount)) || Number(receipt.amount) <= 0) errors.push("treasureReceipt:" + key); });
    const pending = state?.pendingMapEvent;
    if (pending) {
      if (pending.status !== "pending" || !pending.nodeId || !pending.eventId || pending.nodeId !== state.locationId) errors.push("pending");
      if (!Array.isArray(pending.choices) || !pending.choices.length) errors.push("pending:choices");
      if (!MAP_EVENT_TEMPLATES.some((entry) => entry.id === pending.eventId)) errors.push("pending:event");
    }
    Object.values(runtime.nodes || {}).forEach((record) => {
      if (!Array.isArray(record.resolvedIds) || new Set(record.resolvedIds).size !== record.resolvedIds.length) errors.push("resolvedIds");
      if (!Number.isFinite(Number(record.cooldownUntilTurn))) errors.push("cooldown");
    });
  ...
  function rollDefeatBonus(state, info) {
    ...
    if (roll < 30) {
      ...
      gainExp(state, bonus, "combat_insight", { enemyId: info.id });
  ...
  function monsterAction(state, entityId) {
    ...
    const guardStance = companion?.guardStance || "follow";
    const interceptChance = companion && ["active", "mutated"].includes(companion.state) ? (guardStance === "guard" ? 0.45 : guardStance === "protect" ? 0.35 : 0.12) + clamp(Number(companion.loyalty || 0) / 1000, 0, 0.1) : 0;
    const intercept = interceptChance > 0 && replayRandom(state, "combat-target:" + entityId + ":" + Number(state.meta?.turn || 0)) < interceptChance;
    if (intercept && typeof window !== "undefined" && window.GameExpansion?.recordCompanionDamage) {
  ...
  function applyPlayerDamage(state, enemyId, damage) {
    // Overloaded legacy call: expansion systems use (state, amount) for damage
    // to the player. Keep it transactional and consume reviveOnce exactly once.
    if (damage === undefined && Number.isFinite(Number(enemyId))) {
      const amount = Math.max(0, Number(enemyId));
      state.player.hp = Math.max(0, Number(state.player.hp || 0) - amount);
      if (state.player.hp <= 0) {
        const modifiers = typeof window !== "undefined" && window.GameExpansion?.getWorldModifiers ? window.GameExpansion.getWorldModifiers(state, { activity: "combat" }) : {};
        if (modifiers.reviveOnce && !state.flags.reviveOnceUsed) {
          state.flags.reviveOnceUsed = true;
          state.player.hp = Math.max(1, Math.round(Number(state.player.maxHp || 1) * 0.35));
          pushHistory(state, { type: "sys", text: "Một lần hồi sinh đã kéo ngươi trở lại từ bờ vực." });
          return { success: true, damage: amount, revived: true };
        }
        state.pendingEnding = state.pendingEnding || "succumb";
      }
      return { success: true, damage: amount, player: true, defeated: state.player.hp <= 0 };
    }
    const info = combatEntity(state, enemyId);
    if (!info || !Object.prototype.hasOwnProperty.call(state.enemies || {}, enemyId)) return { success: false, reason: "Mục tiêu không còn trong trận.", duplicate: Boolean(info) };
    const amount = Math.max(0, Number(damage) || 0);
    state.enemies[enemyId] = Number(state.enemies[enemyId]) - amount;
    if (state.enemies[enemyId] > 0) return { success: true, defeated: false, damage: amount, enemyId };
    delete state.enemies[enemyId];
    gainExp(state, info.exp, "combat", { enemyId: info.id });
  ...
  function ensureWorldClock(state) {
    ...
    // Canonical adapter: preserve the legacy clock while exposing the nested
    // world-clock shape required by new consumers.
    state.gameClock = state.gameClock || {};
    state.gameClock.world = {
      ...(state.gameClock.world || {}),
      epochDate: "6876-01-01",
      startDayIndex: 2475360,
      currentDayIndex: state.worldClock.absoluteDay,
      currentYear: state.worldClock.currentYear,
      currentMonth: state.worldClock.currentMonth,
      currentDay: state.worldClock.currentDay,
      lastSyncedPlayerDay: playerDay
    };
  ...
  const ERROR_NARRATIVE_MAP = Object.freeze({
    ...
    INVALID_USE_COUNT: ["Số lần sử dụng hiện tại không hợp lệ."],
    MAX_USES: ["Mệnh Số này đã chạm giới hạn số lần sử dụng."],
    RESOURCE_SHORTAGE: ["Tài nguyên hiện có chưa đủ để tiếp tục."],
    COMPANION_ROLE_MISMATCH: ["Vai trò của đồng hành này không phù hợp với hành động."],
    EXPIRED: ["Cơ hội này đã hết hiệu lực."],
    FACTION_REQUIRED: ["Cần gia nhập đúng trận doanh trước khi tiếp tục."],
    FATE_REQUIRED: ["Cần có Mệnh Số phù hợp trước khi tiếp tục."],
    GUILD_RANK_REQUIRED: ["Cấp bậc Bang Hội hiện tại chưa đủ."],
    GUILD_REQUIRED: ["Cần gia nhập Bang Hội trước khi tiếp tục."],
    MAP_VERSION_CONFLICT: ["Bản đồ hiện tại đã thay đổi; hãy tải lại trạng thái hành trình."],
    PATH_MISMATCH: ["Con Đường hiện tại không phù hợp với hành động này."],
    REALM_TOO_HIGH: ["Cảnh giới hiện tại vượt quá giới hạn của hành động."],
    REALM_TOO_LOW: ["Cảnh giới hiện tại chưa đủ để thực hiện hành động."],
    ROUTE_NOT_FOUND: ["Không tìm thấy lộ trình phù hợp."],
    UNKNOWN_COMPANION_SKILL: ["Kỹ năng đồng hành này chưa được định nghĩa."],
    UNKNOWN_TECHNIQUE: ["Công pháp này chưa được định nghĩa."]
  });
  ...
  const NARRATIVE_BANNED_WORDS = /\b(?:Depth|Search|session|lượt dò|counter|index|ID|state|buff|debuff|multiplier|cooldown|Offline)\b/iu;
  ...
  // Player-facing text can come from old saves, imported event payloads, or
  // content authored outside the main bundle. Repair a single mojibake pass at
  // the final boundary so one malformed producer cannot poison the log UI.
  function repairMojibakeText(input) {
    const cp1252Bytes = {
      "€": "\x80", "‚": "\x82", "ƒ": "\x83", "„": "\x84", "…": "\x85", "†": "\x86", "‡": "\x87", "ˆ": "\x88", "‰": "\x89", "Š": "\x8a", "‹": "\x8b", "Œ": "\x8c", "Ž": "\x8e",
      "‘": "\x91", "’": "\x92", "“": "\x93", "”": "\x94", "•": "\x95", "–": "\x96", "—": "\x97", "˜": "\x98", "™": "\x99", "š": "\x9a", "›": "\x9b", "œ": "\x9c", "ž": "\x9e", "Ÿ": "\x9f"
    };
    // Protect valid Unicode punctuation before the legacy mojibake repair.
    // The old byte map treated an actual em dash/ellipsis as CP1252 bytes and
    // later stripped them as C1 controls.
    const protectedText = String(input ?? "").replace(/…/g, "\uE000").replace(/—/g, "\uE001").replace(/–/g, "\uE002");
    const normalized = protectedText.replace(/[\u0080-\u{ffff}]/gu, (char) => cp1252Bytes[char] || char);
    return normalized.replace(/[\u0000-\u00ff]+/g, (run) => {
      if (!/(?:\u00c3(?=[\u0080-\u00ff\u2000-\u206f])|\u00c2(?=[—§·\u0080-\u00ff])|\u00c4(?=[‘’“”†‡\u0080-\u00ff])|\u00c5(?=[\u0080-\u00ff])|\u00c6(?=[\u0080-\u00ff])|\u00e1[»º]|\u00e2€|\u00f0Ÿ)/.test(run)) return run;
      try {
        const bytes = Uint8Array.from([...run].map((char) => char.charCodeAt(0)));
  ...
  function narrativeSafe(text, event = {}) {
    let value = sanitizeLogUtf8(text).trim();
    ...
    // Never expose internal JavaScript producer names in the novel log.
    // Older saves and expansion producers may have persisted values such as
    // resolveMapEvent/bidAuction instead of a player-facing sentence.
    const technicalFunction = /\b(?:resolve|submit|run|accept|bid|refresh|validate|choose|perform|record|update|process|handle|render|create|use|select|move|search|talk|combat|load|save)[A-Z][A-Za-z0-9_]*\b/;
    if (technicalFunction.test(value)) {
      value = "Một rào cản vô hình khẽ khép lại trước hành động của ngươi; hãy thử lại khi hoàn cảnh đổi khác.";
    }
    ...
    const runtimeLint = lintNarrativeText(value, { statDisplay: Boolean(event.statDisplay?.length) });
    if (!runtimeLint.ok) value = "Một rào cản vô hình khẽ khép lại trước hành động của ngươi; hãy thử lại khi hoàn cảnh đổi khác.";
    return value;
  }
  ...
  function groupIntoScenes(history) {
    ...
    (Array.isArray(history) ? history : []).forEach((event) => {
      ...
      const requestedSceneId = String(event.sceneId || dayKey).replace(/:turn(?::|=)[^:|]+/i, "");
      const previous = scenes[scenes.length - 1];
      const sameScene = previous && previous.dayKey === dayKey &&
  ...
  function novelLogParagraphs(state, events = null) {
    const source = (Array.isArray(events) ? events : getGameLog(state)).map((event) => normalizeHistoryEvent(state, event)).filter(Boolean);
    return groupIntoScenes(source).map((scene) => ({
      dayKey: scene.dayKey,
      sceneId: scene.sceneId,
      clock: scene.clock,
      text: scene.texts.join(" "),
      events: scene.events,
      statDisplay: [...new Set(scene.stats)].filter(Boolean),
      portrait: scene.events.find((event) => event.portrait)?.portrait || null
    }));
  }
  ...
  function validateLogSurfaceState(state) {
    ...
    history.forEach((event, index) => {
      ...
      if (text && LOG_UTF8_MOJIBAKE.test(text)) errors.push("utf8:" + index);
      if (text) { const lint = lintNarrativeText(text, { statDisplay: hasStats }); if (!lint.ok) errors.push("narrative:" + index + ":" + lint.issues.join(",")); }
    });
    novelLogParagraphs(state).forEach((paragraph) => seenDays.add(paragraph.dayKey));
    return { ok: errors.length === 0, entries: history.length, paragraphs: novelLogParagraphs(state).length, days: seenDays.size, errors };
  }
  ...
  function renderScene(state, events = []) {
    const scenes = groupIntoScenes((Array.isArray(events) ? events : []).map((event) => normalizeHistoryEvent(state, event)).filter(Boolean));
    return scenes.map((scene) => {
      const body = scene.texts.join(" ");
      const stats = [...new Set(scene.stats)].filter(Boolean);
      return "【" + scene.clock + "】\n\n" + narrativeSafe(body, scene.events[0] || {}) + (stats.length ? "\n◇ " + stats.join(" · ") : "");
    }).join("\n\n");
  }
  ...
  function createGameEvent(state, input = {}) {
    ...
    const event = {
      id: input.id || "evt_" + String(state.meta?.turn || 0) + "_" + state.logState.sequence.toString(36),
      ...
      rawText: input.rawText ?? input.text ?? null,
      locationId: input.locationId || state.locationId || null,
      sceneId: input.sceneId || state.logState.activeSceneId || ("scene:" + String(state.locationId || "unknown") + ":" + gameLogDayKey({ clock: clockLabel(state) })),
      relation: input.relation || "standalone",
      causedBy: input.causedBy || null,
      playerVisible: input.playerVisible !== false,
      debugOnly: Boolean(input.debugOnly),
      errorCode: input.errorCode || null,
      stats: Array.isArray(input.stats) ? input.stats.slice() : [],
      narrativeKey: input.narrativeKey || null,
      npcContext: input.npcContext || null,
  ...
  function normalizeHistoryEvent(state, event = {}) {
    if (!event || typeof event !== "object") return null;
    if (!event.id) {
      state.logState = state.logState || { sequence: 0 };
      state.logState.sequence = Number(state.logState.sequence || 0) + 1;
      event.id = "legacy_evt_" + String(state.logState.sequence).padStart(6, "0");
    }
    event.locationId ||= state?.locationId || null;
    event.sceneId ||= "scene:" + String(event.locationId || "unknown") + ":" + gameLogDayKey(event);
    event.relation ||= "standalone";
    if (event.causedBy === undefined) event.causedBy = null;
    if (event.playerVisible === undefined) event.playerVisible = !event.debugOnly;
    if (event.debugOnly === undefined) event.debugOnly = event.type === "COMMAND_ECHO";
    if (event.errorCode === undefined) event.errorCode = null;
    if (!Array.isArray(event.stats)) event.stats = Array.isArray(event.statDisplay) ? event.statDisplay.slice() : [];
    if (event.rawText === undefined) event.rawText = event.text || null;
  ...
  function moveActions(state) {
    const loc = runtimeLocationPool(state)[state.locationId], world = ensureOpenWorld(state);
  ...
  function contextState(state) {
    ...
    if (pendingOpportunity) {
      ...
      if (!pendingOpportunity.kind && !pendingOpportunity.options && !pendingOpportunity.resolutionOptions) {
        return resolveActionPriority(state, { inCombat: false, forced: false, state: "PENDING_OPPORTUNITY", pending: true, actions: [
          { id: "act_exp_opportunity", label: "Cơ Duyên Hiện Ra", aliases: ["cơ duyên", "co duyen"], priority: 0, tier: 0, urgency: 100, blocking: true, category: "opportunity" },
          { id: "act_trang_thai", label: "Trạng Thái", aliases: ["trạng thái", "trang thai"], priority: 1, tier: 3, scope: "system", consumesTurn: false, category: "utility" }
        ] });
      }
      return resolveActionPriority(state, { inCombat: false, forced: false, state: "PENDING_OPPORTUNITY", pending: true, actions: [
        { id: "act_opportunity_fight", label: "Tranh Đoạt", aliases: ["tranh đoạt", "cuong đoạt"], priority: 0, tier: 0, urgency: 100, blocking: true, category: "opportunity" },
        { id: "act_opportunity_scheme", label: "Dùng Mưu", aliases: ["dùng mưu", "dung muu"], priority: 0, tier: 0, urgency: 99, blocking: true, category: "opportunity" },
        { id: "act_opportunity_share", label: "Chia Sẻ", aliases: ["chia sẻ", "chia se"], priority: 0, tier: 0, urgency: 98, blocking: true, category: "opportunity" },
        { id: "act_trang_thai", label: "Trạng Thái", aliases: ["trạng thái", "trang thai"], priority: 1, tier: 3, scope: "system", consumesTurn: false, category: "utility" }
      ] });
    }
    const pendingExploration = pendingExplorationAt(state);
    ...
    if (!inCombat && (pendingExploration || state.pendingMapEvent?.status === "pending")) {
      ...
      if (state.pendingMapEvent?.status === "pending") pendingActions.push({ id: "act_exp_map_event", label: "Xử Lý Phát Hiện Ẩn", aliases: ["xử lý phát hiện", "xu ly phat hien"], category: "discovery" });
      pendingActions.push({ id: "act_search_leave", label: "Bỏ Qua Phát Hiện", aliases: ["bỏ qua phát hiện", "bo qua phat hien"], category: "interaction" });
      pendingActions.push({ id: "act_trang_thai", label: "Trạng Thái", aliases: ["trạng thái", "trang thai"], priority: 1, tier: 3, scope: "system", consumesTurn: false, category: "utility" });
      return resolveActionPriority(state, { inCombat: false, forced: false, state: "PENDING_EXPLORATION", pending: true, actions: pendingActions });
    }
  ...
  function resolveActionPriority(state, context) {
    ...
    if (result.pending) return { ...result, actions: resolveActions(state, actions) };
    if (state.pendingContestedOpportunity?.status === "pending") return { ...result, actions: resolveActions(state, actions) };
  ...
  const ACTION_DEFAULTS = Object.freeze({ tier: 2, urgency: 0, scope: "local", category: "other", blocking: false, consumesTurn: true, enabled: true, disabledReason: null, source: "engine" });
  function normalizeAction(action = {}, sourceOrder = 0) {
    const a = { ...ACTION_DEFAULTS, ...action };
    if (!action.surface) a.surface = Number(a.tier) >= 3 ? "overflow" : "quick";
  ...
  function resolveAction(state, actionId, options = {}) {
    ...
    if (actionId.startsWith("act_skill_")) return useTechnique(state, actionId.slice("act_skill_".length), options);
    const handlers = {
  ...
  function submitActionId(state, actionId, options = {}) {
    ...
    if (action.enabled === false || action.disabledReason || action.disabled_reason) {
      return { success: false, reason: action.disabledReason || action.disabled_reason || "Hành động chưa sẵn sàng." };
    }
  ...
  function submitTurn(state, action, options = {}) {
    ...
    pushHistory(state, { type: "COMMAND_ECHO", debugOnly: true, playerVisible: false, text: "> " + text });
  ...
  function getChuyenSinhBlockers(state) {
    ...
    if (state.flags?.chuyenSinhCooldownUntil && gameDayIndex(ensureGameClock(state)) < state.flags.chuyenSinhCooldownUntil) blockers.push("Chuyển Sinh đang trong thời gian hồi");
    return blockers;
  }
  ...
  function processChuyenSinh(state) {
    ...
    state.flags = state.flags || {}; state.flags.chuyenSinhPoints = Number(state.flags.chuyenSinhPoints || 0) + 1; state.flags.chuyenSinhCooldownUntil = gameDayIndex(ensureGameClock(state)) + 1;
  ...
  function describeInventory(state) {
    ...
    const item = itemDefinition(state, id);
  ...
  function describeQuests(state) {
    ...
    return "§ Nhiệm vụ:\n" + list.map((q) => {
      const def = questDefinition(state, q.id);
  ...
  function describeMap(state) {
    ...
    const exits = Object.entries(locationExits(state)).filter(([, id]) => id && runtimeLocationPool(state)[id]).map(([dir, id]) => {
      return "  " + directions[dir] + " → " + runtimeLocationPool(state)[id].name;
    });
  ...
  function migrateV12ToV13(state) {
    ...
    state.schemaVersion = Math.max(13, Number(state.schemaVersion || 12));
  ...
  function deserialize(json) {
    ...
    state.generatedItems = state.generatedItems || {};
    state.player.generatedItems = state.player.generatedItems || {};
    Object.assign(state.player.generatedItems, state.generatedItems);
    state.generatedItemSequence = Math.max(Number(state.generatedItemSequence || 0), Object.keys(state.generatedItems).length);
    ensureStarterKit(state);

...
NPC schedules, co-location interactions, and reactions to world conditions are cross-system behavior described by [`WORLD_SIMULATION_CANONICAL.md`](../04-world/WORLD_SIMULATION_CANONICAL.md) and orchestrated in [`WORLD_SIMULATION_CANONICAL.md`](../04-world/WORLD_SIMULATION_CANONICAL.md). NPC event producers follow [`UI_ACTION_LOG_CANONICAL.md`](../07-ui/UI_ACTION_LOG_CANONICAL.md) via [`UI_ACTION_LOG_CANONICAL.md`](../07-ui/UI_ACTION_LOG_CANONICAL.md#unified-novel-style-event-log), retaining `npcContext` and `weatherContext` while keeping raw codes out of Player Log.
...
### Source: `archive-requirements\logic-history\04-interaction\NPC_RUMOR_PROPAGATION_CANONICAL_2026-09-16.md`
...
### Source: `archive-requirements\logic-history\04-interaction\NPC_SCHEDULER_CONGESTION_CANONICAL_2026-09-16.md`
...
### Source: `archive-requirements\logic-history\04-interaction\WAR_FRONT_RUMOR_BULLETIN_VIEWMODEL_2026-09-17.md`
...
### Source: `archive-requirements\logic-history\05-operations\RUMOR_WITNESS_PROPAGATION_POLICY_2026-09-17.md`
...
### Source: `archive-requirements\logic-history\05-operations\WORLD_TICK_NPC_OFFLINE_CANONICAL_2026-09-16.md`
...
## Consolidated addendum: NPC quest and social simulation

- Notable NPCs may move by schedule, maintain bounded `memoryWithPlayer`, exchange rumors, and expose time-limited quests.
- NPC dialogue and quest availability must check current node and sub-location. Weather/world reactions are idempotent by NPC/day/weather key.

### NPC, map, and organization interaction contract

This contract consolidates the remaining root-level NPC interaction proposal. State transitions are owned by the engine and world tick; UI reads projections. Every generated consequence uses a stable event key and the existing history/reward ledger.

#### Schedule, lifespan, migration, and settlement

- A notable NPC schedule is a deterministic list of non-overlapping `[hourStart, hourEnd)` intervals covering the local day, each with a valid `subLocationId` and an activity tag. Resolve the active interval from game time, not wall time. At night, sleeping NPCs expose `wake` in place of ordinary conversation; waking them is a consequential action with detection risk and relationship/faction effects. Schedule evaluation never consumes RNG.
- Store `ageDays`, `birthDay`, and a lifespan resolved once from the NPC's realm and seed. On the first world tick at or beyond `birthDay + lifespanDays`, atomically set `status=deceased`, close active quests, write one node-history death event, and stop schedule/path updates. A living successor is eligible only through an explicit `su_do` relation and valid NPC role. The successor inherits the vacant role, node/sub-location assignment, and only explicitly transferable daily quests; identity, personality, relationships, and personal memory remain separate. With no eligible successor, retain a vacant-role record for succession politics.
- Itinerant routes are selected from node metadata (`seasonalTags`, event tags, valid exits), filtered by current season/event and NPC role. Sort candidates by route cost then node ID and select with the NPC's persisted deterministic seed. Recompute only when season/event changes or the next target becomes invalid. Event couriers spawn at most one per event-region, carry a reference to that event, and leave after its terminal state.
- A footprint is created only when an NPC actually leaves a node. Keep it for three game days; store source node, day, NPC ID, and a deterministic clue class, never the NPC's current destination. `track_footprint` consumes an action, returns a direction or a false lead, and is unique per clue. It cannot disclose current coordinates or bypass fog.
- An itinerant merchant may settle only at an unowned eligible node after at least eight recorded visits across distinct game days. On each subsequent monthly tick, use a stable per-NPC/node roll with a 20% chance to create one small settlement sub-location if the node still has capacity. Save the settlement ID and creation event; uniqueness key is NPC + node. No repeated roll may duplicate the settlement.

#### NPC encounter and social actions

- Keep `trust`, `respect`, and `affection` as separate 0–100 dimensions for this interaction contract; do not derive one from another or substitute global reputation. Apply changes only through idempotent relationship events. The relationship canonical's additional `loyalty`, `fear`, and `suspicion` dimensions remain independent where that policy applies.
- Gift action validates inventory, consumes exactly one item only after a successful transaction, and resolves preference from NPC role/personality tags against item tags. Default deltas: liked +8 affection, neutral +1, disliked −6; duplicates for the same NPC/day/item/action key are rejected. The interaction response discloses the NPC's reaction, not hidden numeric preferences.
- Intimidation is available only when dark reputation is at least 50 and player power exceeds the NPC threshold by the configured margin (default two realm ranks). Resolve one deterministic skill check. Success yields the requested information or item only if the NPC's inventory/knowledge grants it, sets trust and affection to zero for that NPC, and records a faction report with a 30% base chance modified by witness/access. Failure consumes the action, creates no reward, and raises suspicion. It never improves trust or affection.
- A significant public deed emits one rumor fact with source, confidence, creation day, expiry day, and event ID. Witnesses propagate it at most one edge per world tick to eligible co-located/connected NPCs; confidence falls by 20 per hop and rumor lifetime is 30 days. A recipient records the fact once and uses it only to adjust first-meeting attitude (maximum ±10); it cannot overwrite a direct relationship event or persist after expiry. Rumor propagation is deterministic and capped per tick.
- A marked notable NPC may create one trust trial before granting a protected action such as anchoring or teaching a secret technique. Trial has a 14-day deadline, a stable quest ID, and explicit success/failure evidence. Success raises trust by 15 and opens the gated action; failure or expiry lowers trust by 10 and sets a 30-day retry cooldown. Duplicate trial generation is blocked while an active or cooling-down record exists.
- Betrayal is a staged state machine, never an unconditioned random event: `stable → warning → betrayed`. It requires either corruption above that NPC's persisted tolerance or a rival faction offer above the NPC's threshold, plus an active companion/ally relation or trust ≥70. At most one warning is generated; it is observable at least one game day before betrayal. The player can resolve the cause during that window. If unresolved, betrayal changes allegiance/hostility once and emits one history event; it cannot silently delete inventory, quests, or save references.

#### NPC-facing organization systems

- Internal rank is ordered `outer → inner → true disciple → elder → sect leader`. Promotion checks contribution, faction reputation, realm eligibility, and any rank-specific trial from a single `promotionEligibility` resolver. Elders and leaders require an explicit exam/battle result. Failed promotion records a seven-day cooldown; successful promotion unlocks only rank-tagged sub-locations and organization content. Demotion is an explicit event with reason and effective day.
- Organization commissions have `ordinary`, `special`, and `life_or_death` tiers. Ordinary is repeatable under its existing daily limit. Special allows at most two issuances per game month per player and organization. Life-or-death requires an active war in which the organization is a participant. Each commission stores issuer, objective, deadline, reward ledger key, and failure consequence; completion and expiry settle once. Failure of a life-or-death commission may reduce one rank only after a recorded review event.
- Succession politics begins when a leader has at most 30 lifespan-days remaining and no designated successor. Create one crisis with at least two eligible elders, internal tension, and a deadline. A player may support one side only at elder rank or above. Resolution changes leader, faction direction, and stability through one world-simulation transaction; no candidate means the role remains vacant and stability falls by a bounded amount.
- Defection from organization A to its declared rival B requires a separate confirmation after showing consequences. On confirmation, mark the player as a defector to A, update A's reputation once, and make A-affiliated NPCs hostile by policy while retaining historical relationship records. An NPC anchor tied to A receives a warning before any loss/disable transition. A pursuit actor may be spawned once with a traceable origin and a normal encounter/escape/surrender resolution; defection itself cannot erase the anchor silently.
- Secret vault entries carry `organizationId`, minimum rank, and unique content ID. Browsing is read-only; learning requires eligibility and uses the existing technique/fate acquisition transaction. No random drop may grant content marked organization-exclusive.
- Alliance brokering is available only when both faction reputations meet the configured threshold (default 60) and bilateral tension is in the high band (≥70), with neither side already at war. One deterministic diplomacy check uses reputation and Dao Heart. Success reduces tension by 30 and sets a 30-day cooldown; failure increases tension by 5 and sets the same cooldown. It cannot directly cancel an active war.
- Loyalty tests are issued at most once per organization/player per game month when the player holds a rank with duties. The test stores evidence, deadline, and two explicit choices; a hidden morality choice is not allowed. Resolution changes contribution/reputation and may change rank only through the promotion/demotion resolver. Test tasks use the ordinary quest lifecycle and cannot grant rewards twice.

These rules define feature behavior; runtime status remains evidence-driven in `AUDIT_CANONICAL.md` and must not be inferred from the presence of a design contract.

## Trust trial lifecycle

A trial is eligible only for a marked notable NPC and a protected action (anchor, secret teaching, or explicit equivalent). Stable ID: `trust:<npcId>:<kind>:<createdDay>`. At most one active trial per NPC/kind; it cannot be recreated before `retryAfterDay`. State: `active ? passed|failed|expired`; terminal transitions are idempotent. Resolve only with evidence tied to the protected action; an expired trial cannot pass. At `day > deadlineDay`, the daily NPC/world tick expires it, applies trust -10 once, and sets a 30-day retry cooldown. Explicit failure applies the same consequence. Success before deadline applies trust +15 once and unlocks only that action. Deadline is createdDay+14 inclusive. Migrate legacy expiry to createdDay+14 and cooldown to 30 days.

## Loyalty test state machine

Issue only to an active organization member holding a rank with duties, at most once per organization/player per 30-day month. Persist `id`, `status`, `createdDay`, `deadlineDay=createdDay+7`, `investigated`, evidence, and resolution key. Creation is a separate idempotent action; it cannot be combined with a choice. `investigate` records evidence but does not resolve; only after investigation may the player choose `protect_village` or `obey_order`. Resolve once before deadline; update contribution/reputation through event ledgers. Rank changes only through promotion/demotion resolvers. Expiry settles once with no silent demotion. Membership loss expires the task without reward. Migrate legacy investigate-phase records and require evidence first.
